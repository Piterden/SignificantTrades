...
class Sfx {
  ...
  connect () {
    ...
    const filter = new tuna.Filter({
      ...
      filterType: 'highpass', // lowpass, highpass, bandpass, lowshelf,
  ...
  tradeToSong (factor, side) {
    ...
    setTimeout(() => {
      if (side) {
        if (factor >= 10) {
          [659.26, 830.6, 987.76, 1318.52].forEach((f, i, a) => setTimeout(() => this.play(f, 0.05 + Math.sqrt(factor) / 15, 0.1 + factor * 0.1), i * 80))
        }
        else if (factor >= 1) {
        ...
        [493.88, 369.99, 293.66, 246.94].forEach((f, i, a) => setTimeout(() => this.play(f, 0.05 + Math.sqrt(factor) / 10, i > 2 ? 0.1 + factor * 0.1 : 0.2), i > 2 ? 80 * 3 : i * 80))
      }
      else if (factor >= 1) {
  ...
  liquidation () {
    [329.63, 329.63].forEach((f, i, a) => setTimeout(() => this.play(f, 0.5, 0.25, 'sine'), i * 80))
  }
