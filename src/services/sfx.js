...
class Sfx {
  ...
  tradeToSong (factor, side) {
    ...
    setTimeout(() => {
      if (side) {
        if (factor >= 10) {
          [659.26, 830.6, 987.76, 1318.52].forEach((f, i, a) => setTimeout(
            () => this.play(f, 0.05 + Math.sqrt(factor) / 15, 0.1 + factor * 0.1),
            i * 80,
          ))
        }
        else if (factor >= 1) {
          [659.26, 830.6].forEach((f, i) => setTimeout(
        ...
        [493.88, 369.99, 293.66, 246.94].forEach((f, i, a) => setTimeout(
          () => this.play(
            f,
        ...
        [493.88, 392].forEach((f, i) => setTimeout(
          () => this.play(f, 0.05 + Math.sqrt(factor) / 10, 0.1 + factor * 0.1),
          i * 80,
        ))
      }
